...
(function () {
    ...
    try {
      ...
      if (raw) {
        ...
        store = {
          ...
          photoBall: parsed.photoBall || null,
        };
  ...
  function saveStore() {
    ...
    try { localStorage.setItem('pung_store', JSON.stringify(store)); }
      ...
      try { store.photo = null; store.photoBall = null; localStorage.setItem('pung_store', JSON.stringify(store)); }
  ...
  // ----- 공 사진 스킨(원형 클립) -----
  let photoBallImg = null;        // 공 사진 캐시(배경 photo 와 독립)
  function loadPhotoBallImage() {
    photoBallImg = null;
    if (!store.photoBall) return;
    const img = new Image();
    img.onload = () => { photoBallImg = img; };
    img.src = store.photoBall;
  }
  loadPhotoBallImage();

  function drawSkin_photoBall(c, x, y, r, phase) {
    if (photoBallImg && photoBallImg.width) {
      c.save();
      c.beginPath(); c.arc(x, y, r, 0, Math.PI * 2); c.clip();
      // cover 핏: 정사각 영역(2r×2r)을 채우게
      const iw = photoBallImg.width, ih = photoBallImg.height;
      const ir = iw / ih;
      let dw, dh;
      if (ir > 1) { dh = r * 2; dw = dh * ir; } else { dw = r * 2; dh = dw / ir; }
      c.drawImage(photoBallImg, x - dw / 2, y - dh / 2, dw, dh);
      c.restore();
    } else {
      // 플레이스홀더: 회색 원 + 안내
  ...
  const SKINS = [
    ...
    { id: 'photoBall', name: '내 사진', price: 120, draw: drawSkin_photoBall },
  ];
  ...
  function awardCoin() {
    const gain = 1 + Math.floor(stage / 3) + (masterMode ? Math.max(0, stage - TOTAL_STAGES) : 0);
    addCoins(gain);
    popsThisRun += 1;
    if (popsThisRun % 5 === 0) showToast('+' + gain + ' 코인! 🪙');
  }
  ...
  function renderSkinGrid() {
    ...
    for (const skin of SKINS) {
      ...
      const card = buildCard(prev, skin.name, owned, equipped, skin.price, () => {
        if (skin.id === 'photoBall') {
          // 사진 공: 소유 전이면 구매 후, 사진 없으면 파일 선택부터
          if (!owned) {
            if (!buy('skins', skin.id, skin.price)) { showShopMsg('코인이 부족해요'); return; }
            equip('skin', skin.id);
            if (!store.photoBall) { elPhotoBallInput.click(); renderShop(); return; }
          } else if (!store.photoBall) {
            equip('skin', skin.id);
            elPhotoBallInput.click(); renderShop(); return;
          } else if (!equipped) {
            equip('skin', skin.id);
          } else {
            // 이미 장착됨 → 사진 교체
            elPhotoBallInput.click(); return;
          }
          renderShop();
          return;
        }
  ...
  function renderBgGrid() {
    ...
    for (const bg of BGS) {
      ...
      const card = buildBgCard(prev, bg.name, equipped, free, bg.price, () => {
        if (equipped) return;
        if (bg.id === 'photo') {
          // 사진이 없으면 먼저 무료 업로드(적용은 사진 선택 후 다시 눌러 차감)
          if (!store.photo) { elPhotoInput.click(); return; }
          if (!spend(bg.price)) { showShopMsg('코인이 부족해요'); return; }
          store.equipped.bg = 'photo'; saveStore();
        } else if (free) {
          // 기본 다크: 무료로 되돌리기
          store.equipped.bg = bg.id; saveStore();
        } else {
          if (!spend(bg.price)) { showShopMsg('코인이 부족해요'); return; }
          store.equipped.bg = bg.id; saveStore();
        }
  ...
  function buildBgCard(prevCanvas, name, equipped, free, price, onAction) {
    ...
    btn.style.padding = '8px 14px'; btn.style.fontSize = '14px'; btn.style.minHeight = '36px';
    if (equipped) {
      btn.textContent = '적용됨'; btn.disabled = true;
    } else if (free) {
      btn.textContent = '적용';
    } else {
      btn.textContent = '적용 (' + price + ' 🪙)';
  ...
  function renderItemGrid() {
    ...
    card.appendChild(nm);
    // 수량 구매 버튼 3개: +1 / +3 / +5 (각각 가격*수량 spend)
    const btnRow = document.createElement('div');
    btnRow.className = 'stick-buy-row';
    [1, 3, 5].forEach((qty) => {
      const cost = STICK_PRICE * qty;
      const btn = document.createElement('button');
      btn.className = 'btn btn-primary';
      btn.style.padding = '8px 10px'; btn.style.fontSize = '13px'; btn.style.minHeight = '36px';
      btn.textContent = '+' + qty + ' (' + cost + ' 🪙)';
      if (getCoins() < cost) { btn.disabled = true; btn.classList.add('btn-disabled'); }
      btn.addEventListener('click', () => {
        if (!spend(cost)) { showShopMsg('코인이 부족해요'); return; }
        store.sticks += qty; saveStore();
        renderShop();
      });
  ...
  function onPhotoSelected(e) {
    ...
    reader.onload = (ev) => {
      ...
      img.onload = () => {
        ...
        store.photo = dataUrl;
        // 업로드 자체는 무료 — 자동 장착하지 않음. '적용' 버튼을 누를 때 코인 차감.
        saveStore();
        if (!store.photo) { showShopMsg('사진 용량 초과 — 더 작은 사진을'); }
        else { showShopMsg('사진 준비됨 — 적용을 눌러 배경 적용'); }
        loadPhotoImage();
        renderShop();
      };
  ...
  // 공 사진 업로드 → 정사각 크롭 다운스케일(최대 512) → store.photoBall
  function onPhotoBallSelected(e) {
    ...
    reader.onload = (ev) => {
      ...
      img.onload = () => {
        const maxDim = 512;
        const side = Math.min(img.width, img.height);   // 정사각 크롭(중앙)
        ...
        let dataUrl;
        try { dataUrl = cv.toDataURL('image/jpeg', 0.8); } catch (err) { showShopMsg('사진을 읽지 못했어요'); return; }
        store.photoBall = dataUrl;
        saveStore();
        if (!store.photoBall) { showShopMsg('사진 용량 초과 — 더 작은 사진을'); }
        loadPhotoBallImage();
        renderShop();
      };
      img.src = ev.target.result;
